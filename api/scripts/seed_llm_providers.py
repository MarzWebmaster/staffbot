...
Reads API keys from environment variables — NEVER hardcode keys here.
"""
import asyncio, asyncpg, json, os, sys

DB_URL = os.getenv("STAFFBOT_DATABASE_URL", "postgresql://staffbot:staffbot@db:5432/staffbot_db")
...
PROVIDERS = [
    {
        ...
        "description": "Model AI tempatan Malaysia",
        "api_key_env": "ILMU_AI_API_KEY",
        "sort_order": 0,
    },
    {
        "name": "openrouter",
...
async def seed():
    ...
    for prov in PROVIDERS:
        ...
        if not api_key:
            print(f"  ⚠️  Skipping {prov[display_name]} — env var {env_var} not set")
            continue

...
async def customer_page(page: str):
    """Serve customer pages from static/customer/. Auth enforced via frontend JS."""
    # SECURITY: Prevent path traversal — only allow filenames, no slashes/dots
    import re
    ...
    # SECURITY: Resolve real path and verify it's still under static/customer/
    ...
    # Fallback: redirect to customer login if page not found
...
async def admin_page(page: str):
    """Serve admin pages from static/admin/. Auth enforced via frontend JS."""
    # SECURITY: Prevent path traversal
    import re
    if not re.match(r"^[a-zA-Z0-9_.-]+$", page):
        return RedirectResponse(url="/admin/login.html")
    file_path = os.path.join(static_dir, "admin", page)
    # SECURITY: Resolve real path and verify it's still under static/admin/
    real_path = os.path.realpath(file_path)

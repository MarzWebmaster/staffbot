...
@router.post("/internal/track-tokens")
async def track_hybrid_brain_tokens(
    data: dict,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    """Internal: Track token usage from Hybrid Brain (Hindsight LLM calls).

    Called by Hybrid Brain service on Server B when Hindsight uses LLM.
    Deducts tokens from client's managed token quota.
    """
    # Verify request comes from Server B
    api_key = request.headers.get("X-API-Key", "")
    if api_key != settings.SERVER_B_API_KEY:
    ...
    if not client_id or tokens <= 0:
        return {"status": "ignored", "reason": "invalid data"}

    # Find subscription and update used tokens
    from sqlalchemy import select
    from app.models.subscription import Subscription
    ...
    if sub:
        sub.managed_token_used = (sub.managed_token_used or 0) + tokens
        remaining = max(0, sub.managed_token_quota - sub.managed_token_used)

        # Log to usage_log for analytics
        log = UsageLog(
            client_id=client_id,
            total_tokens=tokens,
            operation=operation,
